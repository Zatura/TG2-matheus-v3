...
\usepackage{fixltx2e}         % pacote de fixes latex2
...
%TODO reduzir abstract, tem que ser um parágrafo só, sem citações.
\begin{abstract}
...
\chapter{Introduction} % Matheus: É válido dividir em subseções menores caso necessário?
...
% Exemplo: Uma pessoa está no youtube procurando e assistindo vídeos de implementação de estruturas de dados e programação dinâmica em C++. Logo em seguida, outra pessoa, usando a mesma conta no youtube, passa a procurar vídeos de desenho infantil. Nesse caso, há uma clara quebra de sessão (término da sessão de um usuário e início da sessão de outro usuário, usando a mesma conta).
