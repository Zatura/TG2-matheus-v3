...
\title{From Timeout-based to Item-by-Item Analysis: Splitting User Sessions Originated from Shared Accounts in Online Platforms}
...
% TODO LIST
% Table 4.4 de cutoff dist não tem suas distancias normalizadas
% Melhorar descrição do Capitulo 2
% Subceção 2.2.3 (Cluster analysis) tem só 1 parágrafo
% Melhorar descrição do Capitulo 3 (Methodology)
% Melhorar descrição da Seção 3.3
% 4.6.2 Alternate Least Squares precisa de mais desenvolvimento
% Escrever que o Affinity tem damping 0.9 nos experimentos no capítulo 4
% explicar o Damping na seção de affinity propagation
...
\textit{globo.com} dataset offered a good amount of data, it was preferred instead of others due to it's availability and data abundance, however, since they do not allow the use of news content, \cite{deSouzaPereiraMoreira:2018:CDL:3240323.3240331} made available its embeddings from that carries 250 dimensions of features for each of more than 300,000 articles, also \textit{globo.com} dataset aggregates streams of user clicks with more than 2.8 millions of clicks with labeled information such as user\_id and timestamp, the attribute of stream click is addressed at section \ref{globo_dot_com_dataset}.
...
\chapter{Results} \label{results}
    ...
    \section{Affinity propagation damping factor} \label{affinity_propagation_results}
        ...
        The objective of this metric is finding a lower than 50\% mean error and the lowest \textbf{coefficient of variation} (\(std \div mean\) ). 
